...
namespace DCPUC
{
    public class VariableNameNode : CompilableNode
    {
        ...
        {
            Annotation.register = (int)target;
            ...
            if (variable.location == Register.STACK)
            {
                if (scope.stackDepth - variable.stackOffset > 1)
                {
                    ...
                    AddInstruction("SET", Scope.GetRegisterLabelFirst((int)target), ins, "Fetching variable", Annotation);
                }
                else
                {
                    
                    AddInstruction("SET", Scope.GetRegisterLabelFirst((int)target), "PEEK", "Fetching variable", Annotation);
                }
            }
            else
            {
                ...
                Annotation.register = (int)target;
                AddInstruction("SET", Scope.GetRegisterLabelFirst((int)target), Scope.GetRegisterLabelSecond((int)variable.location), "Fetching variable", Annotation);
            }
